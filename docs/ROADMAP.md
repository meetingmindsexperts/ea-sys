...
|---|---|---|
| 🚧 IN PROGRESS (2026-07-01) | **Build the Docker image in CI → push to ECR → box pulls** (incident action item #2) | **The actual fix for the OOM class + the ~8-min on-box build.** **AWS done:** ECR repo `803726282629.dkr.ecr.ap-south-1.amazonaws.com/ea-sys`, GitHub-OIDC push role `ea-sys-gha-ecr-push` (no stored keys), ECR-pull on the box instance role. **Step 1 SHIPPED** (`a25cc35`): a `build-push` workflow job builds the **web** (`Dockerfile`) + **worker** (`Dockerfile.worker`) images on GitHub runners and pushes them to ECR (`:<sha>` + `:latest`, `worker-<sha>` + `:worker-latest`) — independent of the deploy job, so the box still builds (zero risk while validating). **Step 2 PENDING:** flip `docker-compose.prod.yml` (`build:`→`image:` + keep build for fallback) + `deploy.sh` to ECR-login + `docker compose pull` (with on-box-build fallback), and **run migrations from the pulled worker image** (`docker run <worker-image> npx prisma migrate deploy` — the worker image ships the full node_modules incl. the Prisma CLI, so **DB creds stay in `.env` on the box, never in GitHub**). **Net after Step 2:** SSH deploy drops ~8 min → ~1–2 min; the box never runs a memory-heavy build; rollback = pull a previous `:<sha>`. Source maps still upload from the CI `next build` step. **After Step 2, update the cold-standby runbooks** (recovery becomes pull-not-build). |
