...
| Item | Note |
|---|---|
| **Event-level total cap (Option B)** | The Settings "Maximum Attendees" input persists to `settings.maxAttendees` but is **still unenforced** (pre-existing). A real event-wide cap needs `Event.maxAttendees` promoted to a column + an `Event.seatCount` counter (EventStats is async/fire-and-forget — not race-safe for enforcement) + guards at ~7 claim sites (4 coverable via the seat helpers, 3 hand-placed: public register + 3 import routes). Own reviewed pass; also decide who counts (virtual + faculty excluded per convention; PENDING/WAITLISTED policy). |
