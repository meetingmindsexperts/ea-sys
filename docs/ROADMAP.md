...
- **H2 (correctness — recommended next):** the cert worker's stall-reclaim (`issue-worker.ts` `reclaimStalledRuns`) bounces a stalled `SENDING` run → `AWAITING_REVIEW`, which a manual run resumes via the operator Send click — but an **auto run has no operator**, so a survey-gated run whose email phase stalls >10 min (SES outage / container restart mid-send) is stranded in `AWAITING_REVIEW` un-emailed, and the reg is already terminally stamped so the sweep won't re-enqueue → silent non-delivery. Fix: make reclaim `autoIssue`-aware (keep auto `SENDING` re-drainable, bump `lastTickAt` instead of demoting). ~10 LOC.
...
- **M1 (perf, cheap):** the `AutoIssueAnalyticsCard` polls 8 aggregates every 20s; some (`resolved`/`gaveUp` counts, `recentErrors`, the `certsAutoIssued` join) aren't served by the pending-only partial index → add `(eventId, surveyCompletedAt)` (+ index `CertificateIssueRun.autoIssue`) and bump the poll to 60s.
