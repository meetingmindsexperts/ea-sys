...
| MEDIUM (partial ✅) | **Frontend silent failures** | ✅ **DONE June 26:** registrant portal no longer renders "not registered" on a failed fetch — `e/[slug]/my-registration/page.tsx` now branches on `isError` (red "Couldn't load your registration — your registration is safe" card + Try-again `refetch`) and logs the error to the console/Sentry. **Still open:** bulk-tag failure shows no toast (`bulk-tag-dialog.tsx` + registrations/speakers callers); MEMBER sees write buttons that 403 (registrations list + abstracts) — add a shared `canWrite(role)` gate; several GET-load handlers swallow `!res.ok`. |
