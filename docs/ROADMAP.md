...
|---|---|---|---|
| **Pool reviewers never told there's work; per-abstract reviewers card omits pool members** | HIGH | M | **Partially shipped June 26:** a pre-existing reviewer account added to the pool now gets a `reviewer-pool-invitation` email ("you're a reviewer for X", link to `/my-reviews`) via `notifyReviewerPoolAdded` (new accounts still get the account-setup invite). **Still open:** the per-abstract reviewers card only lists `AbstractReviewer` rows, so organizers can't see pool-reviewer progress per abstract — the card should merge pool members. |
