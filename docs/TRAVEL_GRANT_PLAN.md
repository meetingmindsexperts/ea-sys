...
| Speakers overall with no country (local prod copy, 2026-08-25) | **41 of 117** | Verified while building. Looks alarming and is not: **38 of the 41 are manually-added faculty** who never went through the submitter form, and **none of the 41 has a non-draft abstract**. Only 3 are self-signed-up submitters, all pre-dating the Aug 5 2026 profile gate. |
| Can a NEW abstract author reach us without a country? | **No** | `country` is in the mandatory set in [submitter-profile-completeness.ts](../src/lib/submitter-profile-completeness.ts), which is a HARD 403 gate on submitting an abstract. So the D4 `unknown` bucket is structurally near-empty for the population this feature targets, and the console listing is a safety net rather than a workflow. |
