...
| **Recovery time (RTO)** | **≈ 14 min** detection→recovery (08:59 first errors → ~09:13 back). The **restore operation itself was ~1 min** (`pg_restore` of the 2.56 MB dump at 09:12); the rest of the window was diagnosis + validating the dump in a scratch Postgres (09:09) + a safety-dump of the wiped prod — *not* the DB op. An isolated restore of our data is **~6 s** (re-measured 2026-07-31 via `npm run db:refresh`, 2 orgs / 35 events / 3,592 regs / 99 users — see [ROLLBACK.md §2.1](ROLLBACK.md#21-drill-log--database-restore)). Lesson: the DB rollback is fast; **budget the RTO for diagnosis + validate-in-scratch, and always validate the dump before touching prod.** |
...
- **09:12** — Safety-dumped the current (empty) prod, then `DROP SCHEMA public CASCADE; CREATE SCHEMA public;` + `pg_restore` the 08:00 dump via `DIRECT_URL`. Counts verified. (The `pg_restore` step itself ran in ~1 min — the dump is only 2.56 MB.)
- **~09:13** — App recovered; org-scoped reads return data again, counts match the validated dump. **RTO ≈ 14 min** from first errors.
