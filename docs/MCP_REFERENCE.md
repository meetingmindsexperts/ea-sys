...
- **`create_certificate_template` / `update_certificate_template` gained optional `role` + `cmeHours`** (shipped Phase 1b, additive + backward-compatible). **`role`** is the designation label (drives a `{{role}}` token — e.g. "Speaker", "Moderator"); **`cmeHours`** is static per-template CME hours (drives `{{cmeHours}}`, overriding the event-level value when set; 0–999, pass `null` to clear). CME hours are entered manually per template; issuance stays tag-driven. With per-template uniqueness, this is how you give one person several role certs: make a template per role (each with its own role + cmeHours), tag the person for each, issue each.
- **`create_certificate_template` / `update_certificate_template` gained optional `autoIssueOnSurvey` + `autoIssueTag`** (Phase 2, additive + backward-compatible). When `autoIssueOnSurvey: true` **and** the person holds `autoIssueTag`, the cert is **issued automatically** (rendered + emailed, no operator click) when they complete the event survey — ATTENDANCE templates match the attendee's tags (cert on the registration), APPRECIATION templates match the linked speaker's tags (cert on the speaker). **A tag is required** to match anyone. Idempotent (per-template uniqueness); retry-with-backoff on transient failures; manual Issue stays the backstop for people who surveyed before the template was flagged. `list_certificate_templates` now also returns `role`, `cmeHours`, `autoIssueOnSurvey`, `autoIssueTag`.
...
- **Budget & Procurement (Phase 1, September 14, 2026; the module has been on in production since the same day)**: the read tools `list_budgets`, `get_budget`, `list_spend_requests`, `list_commitments` and, since September 22, 2026, `list_budget_categories` and `list_budget_templates` exist in the code but are NOT in the count above because they register only while `PROCUREMENT_MODULE_ENABLED` is on and only for an OAuth grant whose user may read the module (SUPER_ADMIN, ADMIN, ORGANIZER, MEMBER). **The three budget WRITES (`create_budget`, `add_budget_lines`, `replace_budget_lines`, September 22, 2026) are in-app only:** they register for the Event Agent inside EA-SYS, where the acting person is the signed-in user with a role that may author budgets, and never on this MCP door, whose context carries a role but not a person (the same reason `create_spend_request` is deferred). Package 0.4.39 → 0.4.40 for the two new reads: connected clients reconnect. An API key never sees them: procurement is a per-person surface and a key has nobody behind it, the same rule the HR module applies. With the flag on, an OAuth client sees 120 tools (114 plus the six reads) and must reconnect after each change. Registered from inside the module (`src/procurement/agent-tools.ts`), the CRM pattern. **Revenue and margin (September 17, 2026):** `get_budget` appends the budget's Revenue and margin section (planned and actual per income account, not itemised on won deals, products with no income account, amounts not counted for having no fixed rate, planned and forecast margin, the target) and `list_budgets` shows planned revenue and the target margin, both only for a role with finance sight (every reading role today; the check stays for a narrower role later). Package 0.4.33 → 0.4.34: connected clients reconnect to see the new descriptions.
