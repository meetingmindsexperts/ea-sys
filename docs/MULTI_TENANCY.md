...
- **CRM full-domain sweep** — C1 compound-where on the `Crm*` by-id mutations + `runWithTenant` on `/api/crm/*` + policies for the remaining `Crm*` tables (Company/Deal/Task/Note/Activity/EmailThread/…); CrmContact's policy (Domain #5) is the down-payment.
- **Webinar/Zoom domain (queued July 27, 2026)** — assessed the same day the webinar security batch shipped. **Already tenant-capable functionally:** Zoom credentials are per-org (`Organization.settings.zoom`, AES-encrypted — one of the few subsystems born tenant-ready per IMPACT §6); the recording + attendance workers resolve `event.organizationId` PER ROW and call Zoom with that org's creds; the 4 public webinar routes (`lobby-status`/`stream-status`/`zoom-join`/`presence`) ride `publicEventWhere` from the Phase-0 slug cut (the lobby micro-cache even keys on org); console routes org-bind their event lookups. **What the sweep must add:** (1) **none of the webinar tables carry a direct `organizationId`** — `ZoomMeeting`, `ZoomAttendance`, `WebinarPresence`, `WebinarPoll`/`WebinarPollResponse`, `WebinarQuestion` scope via `eventId` → Event only, so the FLAT RLS policy the swept domains ratified doesn't apply as-is: either add+backfill direct org columns (trivial on the greenfield platform DB) or write join-based policies (a NEW policy shape the harness hasn't ratified — prefer the columns); (2) `runWithTenant` wraps on the webinar routes + `tenantTransaction` on any interactive txns + `check-tenant-als.sh` gate entries; (3) the org-blind worker iterators need the per-row tenant-context wrap the other swept workers get. **Platform-only precondition (same class as the CRM §7.1 global-mailbox note): the streaming stack is a GLOBAL singleton** — one MediaMTX container, one `MEDIAMTX_HLS_URL` / `HLS_CDN_BASE` / `/stream/` path space shared by every tenant; cross-tenant stream isolation is streamKey secrecy only (materially strengthened July 27 when `stream-status` stopped returning the key and gated the HLS URLs on registration). Fine on single-org master; before a second tenant runs a **streamed** (HLS-mode) webinar the platform needs per-tenant publish auth on MediaMTX or per-tenant path prefixes. Zoom-embed-mode webinars have no such precondition.
