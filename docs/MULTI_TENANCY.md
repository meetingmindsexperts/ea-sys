...
- **Core event domains** — Survey, Reimbursement, EmailLog/ScheduledEmail, HelpChatQuery, AuditLog (needs a flat org column). Registration-core + ticketing + finance + CRM + webinar + Speaker + Accommodation + Abstract + Sessions/Tracks + Certificates + **Session Proposals** + **Dinner RSVP** are done.
