...
- **TicketType / PricingTier / PromoCode** — the Registration-core follow-on: add+backfill org columns, stamp the seat/promo-counter writers, policies. Left out of Domain #8 to keep that pass bounded (owner decision).
- **CRM app-wiring (C1/C2/C4)** — the CRM **policy layer is DONE** (Domain #5 CrmContact + Domain #7 the rest of the family); what remains is the app wiring: C1 compound-where on the ~9 gap `Crm*` by-id mutations (deal-service:902, pipeline:390/460/463, product:307/334, deal-document:79/147, import:199/349) + 11 `$transaction` → `tenantTransaction`; C2 `runWithTenant` on the 40 `/api/crm/*` handlers + the 4 inline reads in `src/crm/agent-tools.ts` + **the 2 workers** (`reminders-worker`, `inbound-email-worker`) that run OUTSIDE HTTP where [check-tenant-als.sh](../scripts/check-tenant-als.sh) is structurally blind (out-of-band checklist needed, same class as the invoice cross-domain C2b writers); C4 the gate entries. The Deals cluster is the CRM analog of Invoice's C2a/C2b split (multi-writer hub — sponsor-email, both workers, import, purge, agent-tools all reach in) and warrants adversarial review.
