...
- **Core event domains** — Sessions/Tracks, Certificates, Survey, RSVP, Reimbursement, EmailLog/ScheduledEmail, HelpChatQuery, AuditLog (needs a flat org column), + Session Proposals (already carries an org column — RLS + wraps only). Registration-core + ticketing + finance + CRM + webinar + Speaker + Accommodation + **Abstract** are done.
