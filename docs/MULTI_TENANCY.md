...
**Domain #18 — Comms-log: EmailLog + ScheduledEmail (August 3, 2026, migration `20260803170000`): the first of the entangled trio, unblocked by the owner's "keep them, hidden" decision on NULL-org rows.** **Schema:** both tables already carried `organizationId` (ScheduledEmail non-null + FK'd since birth; EmailLog nullable) — the migration only adds ScheduledEmail's `@@index([organizationId])` (RLS predicate) and **backfills EmailLog's derivable NULL-org rows** (1-hop from the tagged Event, else via the owning Registration/Speaker's swept org columns or Contact's native org); genuinely org-less rows (auth emails to org-null accounts) deliberately stay NULL. **C1 — the logEmail choke point:** EmailLog has ONE writer, so org attribution + the RLS lane are established there for every sender: explicit context org wins, else the org resolves 1-hop from the tagged event (closing the historical several-senders-thread-eventId-but-not-org class behind the `getEmailLogsFor` OR-null fallbacks), and the insert **self-wraps** in `runWithTenant(resolvedOrg)` so stamped rows ride their lane regardless of caller context (auth routes included); failure-isolated (a lookup blip degrades to an un-attributed row, never a lost one). **The asymmetric-policy discovery:** Prisma's `create()` emits `INSERT..RETURNING`, and **RETURNING must pass the SELECT-side USING check** — which deliberately hides NULL-org rows — so the null-org branch writes via `createMany` (plain INSERT, admitted by the WITH CHECK carve-out); pinned in both the unit suite and the harness (a `create()` probe on the same shape is asserted rejected). **C2:** the EmailLog read surface (`/api/email-logs` list + stored-body fetch, both session-org) + the event Email Activity rollup wrapped; the **whole worker `processRow`** (claim / heartbeat / callbacks / terminal writes) now runs on the row's org lane (supersedes the earlier narrow Certificates-H2 wrap around `executeBulkEmail` alone — ALS propagates into the heartbeat timer + fire-and-forget chains); all 4 ScheduledEmail enqueue/management routes wrapped session-org (the two narrow Invoice-C2b precheck wraps superseded by whole-handler wraps); MCP `list/cancel_scheduled_email` wrapped ctx-org (cancel's bare update-by-id also org-bound — defence #1); the `reimbursements/send` **EmailLog dedup read** wrapped resource-org (it fail-closed while its candidate read succeeded → would have RE-MAILED the whole batch — the fail-closed-read-feeding-a-terminal-action class); the survey-thankyou sweep's EmailLog dedup documented as inseparable from its org-blind candidate scan (both fail-close together; never split their lanes). `webinar-email-sequence.ts` verified runs-inside-wraps (sequence route + provisioner + public register all wrap). **Worker/ops preconditions (documented, not fixed):** the scheduled-emails tick scan + stuck sweep, the **email-log-prune job** (under RLS it would silently see zero rows and STOP PRUNING → unbounded htmlBody growth), and the SUPER_ADMIN ops reads in `infra/aws-ops.ts` are org-blind by design — the platform runs them on a privileged maintenance lane. **C3** — [prisma/rls/emaillog.sql](../prisma/rls/emaillog.sql): EmailLog's **asymmetric** policy (`WITH CHECK (org IS NULL OR org = setting)`, strict USING) + ScheduledEmail's flat policy; a 10-assertion [emaillog-rls.test.ts](../tests/tenancy/emaillog-rls.test.ts): SHARED `to` address lane-scoped (no per-org unique field — the MediaFile shape), both asymmetry halves (bare createMany admitted → invisible from BOTH lanes AND bare reads; the create()-RETURNING rejection pinned), smuggled-stamp rejection (the carve-out is no bypass), the **worker CLAIM shape** cross-tenant miss (`updateMany PENDING→PROCESSING` on B's row under A → count 0, B still claimable), fail-closed ×2 WITH the null row present, org-re-home block, DELETE P2025. Harness 272 → 282. **C4** — [check-tenant-als.sh](../scripts/check-tenant-als.sh) gains the `email-logs` dir + the 5 emails/email-activity route files + `email-log.ts` (module) + both models. Full gate green. Behavior-preserving on master. **Adversarial review round (Aug 3, 2026 — 3 lenses: lane correctness / policy+data integrity / master behavior+drift; 0 BLOCKER, 1 HIGH, 8 MED, ~18 LOW; no cross-tenant leak, nothing master-affecting; verified-then-fixed same day):** **(H1)** the asymmetry test proved nothing about `WITH CHECK` — verified empirically by loosening the policy to `WITH CHECK (true)`: all 10 assertions still passed, because the `create()` smuggle is ALSO rejected by USING on its RETURNING clause; fixed with a **createMany smuggle assertion** (plain INSERT — only WITH CHECK can reject it; fails against the broken policy, passes against the real one) + an EmailLog org-re-home UPDATE assertion. **(M)** `logEmail` gained the **ambient-lane fallback** (`context org → event 1-hop → getTenantOrgId()`, `||` so a worker's `""` never stamps) — the event lookup runs on the current lane, so under an Event policy it silently nulls for unwrapped callers, and the code must never depend on "Event never gets a policy" (the harness already policies Event); plus two new warn paths per the every-failure-logs rule: `event-not-found` (stale/deleted eventId — the un-thrown branch the first catch missed) and `unattributed-tenant-row` (a REGISTRATION/SPEAKER/CONTACT row landing NULL-org is lost attribution the carve-out would otherwise swallow — prod measured 85 of 91 null rows were this class, NOT auth emails). **(M)** the "documented" claim about `infra/aws-ops.ts` was drift — the file had zero tenancy comments; now annotated (the compound failure: under RLS a fully stalled queue reports "due 0 / stuck 0" — green); the prune-worker header now states its privileged lane is MANDATORY (null rows are unreadable/undeletable from every lane; it is the pool's only reaper). **(M, measured)** the migration's four backfill UPDATEs matched **ZERO rows on prod** — app-side stamping had already covered the derivable class; the backfill stays as insurance for legacy-data platform migrations, and the earlier "closes the eventId-without-org class" claim was corrected in the policy file. **(M)** unit tests now pin the LANE, not just the stamped column (`getTenantOrgId()` captured inside the mocked writes — a wrong wrap org now fails; the gate only catches a deleted wrap). Riders: worker `onBatchEmailed` update org-bound (defence-#1 parity with the MCP cancel), rejection-probe ids added to the seed cleanup (a future policy regression surfaces as a clean assertion failure, not a P2002 on an invisible row), the stale "EmailLog is unswept" comment in rsvp-invites/send corrected, `getEmailLogsFor`'s OR-null block documented as a DEAD branch under platform RLS. The review's "adjacent pre-existing" find — `runWithTenant(session.user.organizationId ?? "")` giving an org-null SUPER_ADMIN an empty-string lane (everything fail-closes to 404/empty) — turned out on inspection to be a **16-instance class** across the Invoice + Reg-core sweeps, not one route; fail-closed direction, platform-only, deferred as its own pass (ROADMAP §"Comms-log sweep — deferred decisions") rather than point-fixed inconsistently. **Deferred decisions recorded in ROADMAP §"Comms-log sweep — deferred decisions":** tenant-offboarding vs the SetNull org FK (a deleted org's whole email log falls into the invisible pool) and a NULL-org retention/purge job.

**Sweep queue (not yet swept):**
**Domain #19 — AuditLog (August 3, 2026, migration `20260803180000`): the widest writer surface in the codebase — 163 `auditLog.create` sites across 106 files (an inventory agent's paren-balanced sweep; the initial grep undercounted at 59) — swept WITHOUT migrating a single call site, via a central auto-stamp (owner decision: extension + ~18 explicit stamps, over a writeAudit()-helper migration of all 163).** **Schema:** `AuditLog.organizationId` — a bare scalar with **NO FK by design** (the audit trail must survive org deletion with attribution intact; the eventId FK's SetNull already demonstrates the failure mode — 17 of prod's 129 org-less rows were deleted-event orphans whose org linkage the cascade erased) + `@@index([organizationId, createdAt])` for the global feed. **Backfill** derives from 9 sources strongest-first (eventId 1-hop; `changes->>'organizationId'`; `entityId 'org:%'`; Organization-entity rows where entityId IS the org id; User/Contact/Crm*/ScheduledEmail/Registration joins) — prod measured ~95% derivable, residue = deleted-event orphans + org-null-actor auth rows, honestly NULL. **C1a — `withAuditOrgStamp`** ([src/lib/db.ts](../src/lib/db.ts)): a per-model Prisma client extension on `auditLog.create`/`createMany`, chained OUTER to `tenant-set-local` (so the SET LOCAL batch receives already-stamped args and its `query(args)` stays a raw engine PrismaPromise — an inner async handler would break the batch form); resolution = explicit-in-data → ambient tenant lane (`getTenantOrgId()`, `""` from the legacy `?? ""` wrap class correctly skipped) → eventId 1-hop on the RAW base client; **deliberately NOT gated on RLS_SET_LOCAL** (the column must populate on master now) and **deliberately does NOT copy the `inTenantTx` skip guard** (the 10 `tx.auditLog.create` sites must stamp too); never throws (a lookup blip degrades to an unstamped row, never a lost audit write, warn-logged). Verified **empirically against the local DB before any tests**: ambient / 1-hop / explicit-wins / honest-null / tx-path all stamp correctly. **C1b — explicit stamps** at the org-scoped no-eventId writers the extension can't reach without ambient context: `audit-data-transfer.ts` (org param already existed — now written to the column, covering its 30 export/import callers), the 5 CRM `writeAudit` helpers + their 16 call sites (org threaded through the entry type), `billing-account-service` ×3 (org was in `changes` only), the 3 auth routes (`user.organizationId` — org-null actors correctly stamp null), and the 6 org-admin sites (session org). **C2 — readers:** the global `/api/activity` feed's dual-shape OR (`event: { organizationId }` + `changes` JSON leg) replaced by the **flat `{ organizationId }` predicate** — a strict superset that also FIXES a real bug the inventory surfaced: **Contact audits, all 5 CRM config helpers, and org-admin user audits carried no org marker anywhere and were invisible in the global feed**; they now appear (a desirable visible change on master). The `?eventId=` filter keeps the org predicate (narrows within, never leaks). Wraps added: global feed (session org), event activity feed (resource org), the analytics route + MCP `get_event_analytics` (both read AuditLog CHECK_IN rows — resource/ctx org; under RLS an unwrapped analytics read fails closed to an all-zero dashboard). The per-entity `activity-feed.ts` readers were already inside their routes' wraps. **C3** — [prisma/rls/auditlog.sql](../prisma/rls/auditlog.sql): the **asymmetric** EmailLog-shaped policy (WITH CHECK admits NULL — org-null actors' audit rows; USING strict — NULL rows visible to no lane), with the header carrying the sweep's sharpest edge: **ALL 163 writers use `create()` (INSERT..RETURNING), and RETURNING must pass USING — so from an app-role lane with no ambient org, a NULL-org audit write is REJECTED and the fire-and-forget catch logs a LOST row on the platform** (master unaffected; the org-null auth-audit precondition is an owner decision recorded in ROADMAP). A 10-assertion [auditlog-rls.test.ts](../tests/tenancy/auditlog-rls.test.ts): SHARED entityType/entityId pair lane-scoped (the activity-feed query shape), **the auto-stamp LIVE end-to-end** (an org-less `create()` inside A's lane comes back stamped A through extension + policy + pooler), both asymmetry halves incl. the pinned `create()` RETURNING rejection, the createMany WITH CHECK proof (the Domain-#18 lesson applied from day one), explicit-beats-ambient pinned via the smuggle rejection, org-re-home block, fail-closed with the null row present, DELETE P2025. Harness 284 → 294; +8 unit tests ([audit-org-stamp.test.ts](../__tests__/lib/audit-org-stamp.test.ts)) pin the resolution ORDER + failure semantics (empty-string guards, lookup-failure-never-throws). **C4** — [check-tenant-als.sh](../scripts/check-tenant-als.sh) gains the 3 reader routes. Full gate green. Behavior-preserving on master except the two intentional improvements: the flat column populates from deploy, and the global feed shows the previously-invisible org-scoped audits.

- **Remaining domain — the last one, blocked on a recorded decision:** HelpChatQuery (SUPER_ADMIN global read conflicts with RLS — the act-as-tenant decision). Registration-core + ticketing + finance + CRM + webinar + Speaker + Accommodation + Abstract + Sessions/Tracks + Certificates + Session Proposals + Dinner RSVP + Survey + Reimbursement + Comms-log (EmailLog/ScheduledEmail) + **AuditLog** are done.
