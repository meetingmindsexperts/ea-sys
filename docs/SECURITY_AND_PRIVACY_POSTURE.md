...
| **0.2** | **The server's disk is not encrypted at rest.** Verified: the EC2 root volume (`vol-073ca563deaa8732a`, 50 GB) has `Encrypted: false`. Uploaded files live on it. **No passport or bank document has ever been uploaded** (verified 2026-08-19, §2), so this is currently a gap in the *control*, not an exposure of sensitive data — but it must be closed before such documents are collected. Two remediation routes, one of which is free and immediate (§3). | "Encryption at rest" is a standard yes/no on these forms. Today the honest answer for uploaded files is **no**. The database and the DR backups *are* encrypted (§3). |
...
| **Uploaded files on the server** | **No** | EC2 root volume is unencrypted — see §0.2. This includes passport scans and bank-detail documents |
...
2. **Encrypt the server disk** (§0.2, route 1). Removes the last "no" on encryption at rest. Requires 30 to 60 minutes of planned downtime, and rollback is reattaching the original volume.
