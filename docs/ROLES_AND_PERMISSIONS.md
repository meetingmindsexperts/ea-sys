...
- **Operator surfaces** (`denyNonOperator`: SUPER_ADMIN only, and org API keys refused): Logs and the log archive, ID lookup, Backups, Help queries, Agent messages, the Docs viewer, alert silencing, the organisation list. Infra / Ops opens for ADMIN with its own org's scope and for the operator across tenants.
