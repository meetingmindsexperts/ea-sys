...
- **Operator surfaces** (`denyNonOperator`: SUPER_ADMIN only, and org API keys refused): Logs and the log archive, ID lookup, Backups, Help queries, Agent messages, the Docs viewer, alert silencing, the organisation list. Infra / Ops opens for ADMIN with its own org's scope and for the operator across tenants. **Shared doc links** (`/admin/docs/<path>`, one doc per URL, no listing) also open for ADMIN where the deployment sets `ADMIN_DOC_LINKS_ENABLED=true` (master only, Sep 24 2026); the Docs viewer itself stays operator-only.
