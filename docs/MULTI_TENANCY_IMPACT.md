...
- **✅ DECIDED Sep 2, 2026 — [PLATFORM_DECISIONS.md](PLATFORM_DECISIONS.md) §9.** Not a shared identity with per-tenant config sets: **one AWS account per tenant** under AWS Organizations, MMG-created and MMG-paid. Config sets and dedicated IPs were considered and rejected because SES quota and complaint-rate enforcement are **account-level** (AWS can suspend a whole account regardless of internal partitioning). There is **no shared tenant-mail sender at all**: a tenant with no configured sender is refused, and a verified test send is a hard gate on onboarding. Sender resolution moves Event → Organization as this bullet says, via `settings.email` and an `EMAIL_ENV_FALLBACK_ORG_ID` allow-list-of-one mirroring the shipped Stripe pattern.
...
- **Worker (Med / M):** `scheduled-emails` drains globally FIFO (`take MAX_PER_TICK`) — one tenant queueing thousands **starves all others' due emails**. Add fair-share (cap N/org/tick). ~~**Must** fix the advisory-lock pooler-mode caveat (point worker at `DIRECT_URL`, session mode) **before** running a 2nd worker (DR/scale) or risk double-sends.~~ **RESOLVED Aug 10, 2026:** advisory locks were replaced by an expiring **`JobLease`** (`worker/lib/job-lease.ts`), whose claim is a single atomic statement, so correctness no longer depends on which connection ran it. Note the struck advice was itself wrong: pointing the worker at `DIRECT_URL` was tried and did **not** fix it, because the connection split happens inside Prisma's own pool, below the pooler. A second worker is now safe from the double-send angle; the fair-share point above still stands, and the lease key is a `String` primary key so sharding it (`${job}:${shard}`) needs no migration.
...
**Phase 2 — domain-by-domain isolation sweeps:** per domain: `organizationId` column → org-bind queries → enable RLS → domain isolation tests green. Pilot **Contacts** (full sweep, July 23), then blast-radius order (§5); **MediaFile** RLS policy + harness landed July 24 (policy-only — the DB backstop; the already-org-keyed trivial case, route wiring deferred); **BillingAccount** full sweep landed July 24 (C1 compound-where + tenantTransaction, C2 runWithTenant on all 8 handlers, C3 flat RLS policy + harness — the first finance domain); **Invoice** full sweep landed July 27 (the second finance domain + the biggest surface — 13 writers; C1 service org-binding + tenantTransaction, C2a 8 staff routes + 4 MCP executors, C2b narrow wraps on the 6 cross-domain money-path writers incl. the Stripe webhook + refund service, C3 flat RLS policy + harness 42→51; the 3 REGISTRANT invoice/quote reader routes deferred to the Phase-1 identity decision); **Webinar/Zoom** full sweep landed July 28 — the first domain from the "25 tables without an org column" set: migration `20260728140000` denormalized `organizationId` onto all 6 webinar tables (nullable + backfill, ratifying columns-over-join-policies), writers stamp + sync fns self-heal it, 37 `runWithTenant` wraps (12 staff route files + 5 public session routes + 5 MCP executors + per-row worker fns), `prisma/rls/webinar.sql` (first multi-table policy file) + a 10-assertion harness test, and 4 in-band IDOR fixes (the zoom session route's sessionId-only lookups could read/update/DELETE another org's meeting incl. the remote billable Zoom meeting); the whole **CRM (`Crm*`) family** got its RLS policy layer July 27–28 (Domain #5 CrmContact + #7 Groups 1–2, harness 60→147; C1/C2 app-wiring queued); and **Registration-core** — the biggest domain — landed July 28–29 (Domain #8): migration `20260728160000` denormalized `organizationId` onto Registration/Attendee/Payment/RefundAttempt/RegistrationSerialCounter (Attendee stamped only when all its registrations agree on one org — it has no event link — and the serial counter gets a flat column to avoid the ON-CONFLICT unique-violation-misread hazard), 22+2 `$transaction`→`tenantTransaction`, ~28 route + 7 MCP + 3 per-row-worker wraps, `prisma/rls/registration.sql` + an 11-assertion harness test (harness 157→168), and 7 in-band IDOR-fix clusters incl. the public-register orphan-reuse cross-org PII-overwrite; the 7 REGISTRANT self-service routes + sign-in auto-link deferred to the identity decision, and TicketType/PricingTier/PromoCode carved off as a bounded follow-on (the **ticketing follow-on** landed July 30 — migration `20260730120000` org-stamped all 5 ticketing tables incl. the `PromoCodeTicketType` link, 11 CREATE-site stamps + 17 `runWithTenant` wraps on the management CRUD routes + promo MCP executors + narrow event-create/clone wraps, `prisma/rls/ticketing.sql` + an 11-assertion harness test (174→185), closing Domain #8's deferred seat/promo-counter RLS backstop; the counter writers themselves already ride the registration flow's Domain-#8 wraps). Close the `organizationId!` IDOR class. **No external tenant onboards until the domains they touch are proven.**
