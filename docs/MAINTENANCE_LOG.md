...
- [x] **Lifecycle for noncurrent versions** on `ea-sys-uploads`: rule `expire-noncurrent-versions` created Sep 7, 2026 (verified): noncurrent versions expire after **90 days**, expired delete markers removed, incomplete multipart uploads aborted after 30 days. Current versions are never touched; beyond 90 days the never-deleting Singapore mirror is the long-term undo. Every check on the infra card's Uploads storage configuration list is now green.
