...
### 5k stream delivery
HLS playback is **CDN-aware** via `HLS_CDN_BASE` (CloudFront) with the app origin as fallback (emitted by `stream-status` + `zoom-join`); `stream-status` keeps probing the MediaMTX origin internally (cached 3s so 5k pollers share one probe).

**`stream-status` is credential-gated (July 27, 2026 — review M3):** the HLS URL embeds the `streamKey`, which is ALSO the RTMP **publish** credential on MediaMTX — so the route no longer returns `streamKey` to anyone, and the `hlsUrl`/`hlsOriginUrl` fields are returned only to an authenticated caller who is org staff of the event's org or holds a non-cancelled registration (60s per-(user,event) positive micro-cache keeps recovery-poll storms off the pool). Anonymous callers get `{ status }` only — the liveness flag the lobby needs. The LivePlayer fetches same-origin with cookies, so real attendees pass transparently. Additional July 27 hardening: `lobby-status` 404s non-WEBINAR events; `provisionWebinar` claims a `webinar.provisioningAt` sentinel (no more double-provision race); a session retime now re-syncs the linked Zoom meeting's start/duration (`zoomSync` on the session PUT/MCP responses); HLS viewing mode is validated at save AND room-open (`HLS_STREAM_NOT_CONFIGURED`); the console shows a "room still closed — N waiting" alert past the scheduled start plus a DRAFT-auto-open hint. `LivePlayer` fails over CDN → origin once on a fatal error, then resumes the recovery poll (auto-reconnect) rather than dead-ending. See `LIVE_STREAMING.md §13` for the operator-run CloudFront + Singapore-DR failover provisioning.
...
| Operator wants to enable live streaming on an existing session | **Not possible via edit.** The `PUT .../sessions/[id]/zoom` handler is a *refresh* (it re-syncs passcode/joinUrl from Zoom) and never touches `liveStreamEnabled` or `streamKey`. Streaming is minted at CREATE only, so you must delete the Zoom meeting and create it again |
| Operator tries to delete the anchor session to recreate it | Refused: `Refused to delete the webinar anchor session`. `settings.webinar.sessionId` points at it, so deleting it strands the webinar. **Delete the Zoom meeting instead**, the red trash icon inside the session's Zoom panel, which leaves the session intact |
| Recreating the anchor's Zoom meeting picks the wrong type | Refused since Aug 19 2026 (`WEBINAR_ANCHOR_REQUIRES_WEBINAR`). The create form's type comes from `Event.settings.zoom.defaultMeetingType`, which used to be a flat `"MEETING"` **even on WEBINAR events**, so the documented recovery above silently downgraded a webinar unless the operator noticed a dropdown. A downgrade breaks panelists (the panelists route refuses `MEETING`) and all analytics (`/report/webinars/...` does not exist for meetings). The default now derives from `eventType`; the guard is scoped to the anchor only, so a rehearsal meeting on another session is still fine |
| Meeting SDK app deleted in the Zoom Marketplace | Every SDK join fails `3712 Signature is invalid` and **nothing notices until an attendee tries to join**, possibly months later. Zoom offers no endpoint to validate SDK credentials, so there is no health check that would catch it. Clear the SDK fields rather than leave dead values: empty fields fall back to `mode: "url"` (opens in Zoom) and work, wrong fields do not |
