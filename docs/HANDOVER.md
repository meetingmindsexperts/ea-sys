...
- **CI → ECR deploy pipeline (Step 1) + prominent overview Invoices + payment-email polish + Stripe live-mode setup** (July 1) — **CI→ECR (Step 1 shipped, `a25cc35`, validated):** a `build-push` GitHub Actions job now builds the web (`Dockerfile`) + worker (`Dockerfile.worker`) images and pushes them to **ECR** (`803726282629.dkr.ecr.ap-south-1.amazonaws.com/ea-sys`, `:<sha>`/`:latest` + `worker-*`) via **GitHub OIDC** (no stored AWS keys). It's independent of the deploy job — the box **still builds on-box for now**, so Step 1 was zero-risk. **Step 2 (pending):** flip `docker-compose.prod.yml` + `deploy.sh` to `docker compose pull` (with on-box-build fallback) and run migrations from the pulled **worker image** (ships the Prisma CLI → **DB creds stay in `.env`, never in GitHub**), dropping the SSH deploy from ~8 min → ~1–2 min. AWS setup done: ECR repo, OIDC push role `ea-sys-gha-ecr-push`, `ecr-pull` on the box role. **DR impact:** faster box-rebuild RTO (pull vs build); a full-Mumbai-region loss needs **ECR cross-region replication → Singapore** (tracked in ROADMAP). See [docs/AWS_OPERATIONS.md](AWS_OPERATIONS.md) §5.x + the ROADMAP "CI → ECR" item; **the cold-standby runbooks need a pull-not-build update once Step 2 lands.** Also this session: **Invoices** moved off the sidebar to a prominent finance-gated CTA on the event Overview; the confirmation email's **payment-pending block** gained bank-transfer/quote + "don't share this link" + "complete only upon payment" copy; the **payment-confirmation** email's receipt button relabeled "View Receipt" + a note that the invoice arrives separately; and **Stripe was set up for live-mode** (operator swapped `sk_live_` keys + a live webhook — verify the box `.env` is `sk_live_` before HEMNET; test-mode was in place during dev).
