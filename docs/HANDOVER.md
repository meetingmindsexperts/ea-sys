...
If you're returning to this doc after a gap, these are the significant features that landed between April 2 and June 29 and may not be fully reflected in every section below. The deltas are called out inline where relevant, and `CLAUDE.md` is the canonical log.

- **Abstracts submitter flow — details landing page, editable guidelines, co-authors, presentation types, 300-word cap** (July 2) — Returning submitters now land on a **details page** (`/events/[eventId]/abstracts/profile`: profile + linked registration + their abstracts + Submit CTA) after sign-in, backed by an ownership-scoped `GET /abstracts/my-profile`; sign-in landing wired there (also fixed the old SUBMITTER→/events / REGISTRANT→signup redirect bug). **Editable per-event submission guidelines** (`Event.abstractGuidelinesHtml` + `DEFAULT_ABSTRACT_GUIDELINES_HTML` fallback; `{{contactEmail}}` merged from `emailFromAddress`) shown on `/abstracts/new` + the profile page, editable via Content → Abstracts. **Co-authors** (up to 20: name*/email/phone/jobTitle/organization/country) via a new `Abstract.coAuthors` JSON column + shared [abstract-coauthors.ts](../src/lib/abstract-coauthors.ts) + `CoAuthorFields`. **Presentation types** = Oral/Poster (combined, stored as `ORAL` — no enum migration; legacy `POSTER` kept), Video, Workshop. **300-word cap** on the abstract body (server Zod `.refine` + client live counter/disable via [abstract-content.ts](../src/lib/abstract-content.ts); title/authors excluded). Two additive migrations (`20260702150000` guidelines, `20260702160000` coAuthors — blue-green safe, deploys verified green). Shipped `a6ff5ca`/`10a2046` (+ word-limit follow-up). **Same-day feedback round (`a7c0db3`):** co-authors dropped email/phone + split name → First/Last; presentation types = "Oral or Poster" (new combined `ORAL_POSTER` enum value + migration) / Oral / Poster / Video / Workshop, now **mandatory to submit**; **authors can only edit/withdraw while DRAFT** (submitted = locked, "contact the organizer" notice); Specialty removed from the submit page. Full detail in CLAUDE.md July 2 entries; the REVISION_REQUESTED-editing consequence is tracked in ROADMAP.
- **Attendee "Role" recorded everywhere + shown across the dashboard; Identity & Roles runbook; events default ordering; import-contacts pricing tier** (July 2) — The `AttendeeRole` profession field (`Attendee`/`Speaker`/`Contact.role`) was collected on the public forms but rendered nowhere in the dashboard. Now recorded on every create/edit path (companion registration inherits the speaker's role; registration/speaker/contact PUT editable + Contact-synced; CSV import + admin Add forms — which also silently dropped `title`, fixed) and shown everywhere (detail sheets/pages + list columns + registrations CSV + abstract author line) via `formatAttendeeRole()` in [src/lib/schemas.ts](../src/lib/schemas.ts). In the shared `PersonFormFields`, Role sits beside Specialty + Bio beside Photo so Add Registration ↔ Add Speaker match. Same session: new [docs/IDENTITY_AND_ROLES.md](IDENTITY_AND_ROLES.md) operator runbook ("one person, many hats" — facets stack by email, single `User.role` is the only colliding seam, staff-are-separate rule); the **events list default view** now pins **PUBLISHED first, then upcoming** (client-side, explicit column sorts respected); and **Import Contacts → Registrations** now prompts for a **required pricing tier** when the type has tiers + stamps `pricingTierId`+`originalPrice` (closing the import-contacts leg of the "unstamped import paths" finance gap; CSV/EventsAir still deferred). No schema/migration; no MCP change. Shipped `5de8375`/`3b2f515`/`132420e`/`e55f094`/`fa36ba7`. Full detail in the CLAUDE.md "Recent Features" July 2 entries.
- **Promo codes on existing registrations + discount consistency + "Free registration" fix** (July 1) — Organizers (registration detail sheet → Billing) and registrants (`/my-registration` pay-later) can now **apply/remove a promo code on an existing unpaid registration**; the discount stores on `Registration.discountAmount`+`promoCodeId` (which checkout/quote/invoice already read). New [src/services/promo-code-service.ts](../src/services/promo-code-service.ts) (sentinel-rollback tx, unpaid-only, replace-not-stack, atomic `usedCount`, per-email limit) + org route (`denyReviewer`+`denyFinance`) + registrant route (ownership). Follow-ons the same day: the **discount now shows in the quote PDF total + confirmation email** (both previously showed the pre-discount price while charging the discounted amount); the **Billing summary shows Promo/Discount/Total/Outstanding**; **admin resend of the confirmation now attaches the quote + a valid payment block**. Adversarial review → fix-then-ship (0 blockers); two review findings fixed: **HIGH #1** — tier-priced (base 0, no tier) + VIRTUAL regs no longer show "Free registration" (`Registration.originalPrice` is now stamped at create + read first everywhere via `resolveRegistrationBasePrice`/`readRegistrationBasePrice`; **no backfill — forward-looking**); **HIGH #2** — per-email `maxUsesPerEmail` TOCTOU race closed with a transaction-scoped `SELECT … FOR UPDATE` row lock (no schema change). **Deferred review findings tracked in [docs/ROADMAP.md](ROADMAP.md):** 100%-off self-remove dead-end, `discountValue` clamp, register-route reorder, MEMBER-sees-promo-code product call, 3 unstamped import create-paths. See the CLAUDE.md "Recent Features" July 1 entry for the full detail.

- **CI → ECR deploy pipeline (Step 1 + Step 2, cutover complete) + prominent overview Invoices + payment-email polish + Stripe live-mode setup** (July 1) — **CI→ECR (shipped, `a25cc35` + `e118830`):** a `build-push` GitHub Actions job builds the web (`Dockerfile`) + worker (`Dockerfile.worker`) images and pushes them to **ECR** (`803726282629.dkr.ecr.ap-south-1.amazonaws.com/ea-sys`, `:<sha>`/`:latest` + `worker-*`) via **GitHub OIDC** (no stored AWS keys); the deploy job `needs: build-push` and passes `IMAGE_TAG=<sha>` so the box `docker compose pull`s those exact images instead of building on-box (`docker-compose.prod.yml` has `image:` + a kept `build:` fallback; `deploy.sh` ECR-logs-in + pulls, falling back to an on-box build if the pull fails). Migrations run **from the pulled worker image** (ships the Prisma CLI → **DB creds stay in `.env`, never in GitHub**); a migration failure aborts before the nginx swap (old slot keeps serving). **Net:** SSH deploy ~8 min → ~1–2 min, box never runs a memory-heavy build (the INC-001 fix), rollback = `IMAGE_TAG=<old-sha> bash scripts/deploy.sh`. AWS: ECR repo, OIDC push role `ea-sys-gha-ecr-push`, `ecr-pull` on the box role. **DR impact:** faster box-rebuild RTO (pull vs build); a full-Mumbai-region loss needs **ECR cross-region replication → Singapore** (tracked in ROADMAP). See [docs/AWS_OPERATIONS.md](AWS_OPERATIONS.md) §5.x + the ROADMAP "CI → ECR" item; **the cold-standby runbooks still need a pull-not-build refresh.** Also this session: **Invoices** moved off the sidebar to a prominent finance-gated CTA on the event Overview; the confirmation email's **payment-pending block** gained bank-transfer/quote + "don't share this link" + "complete only upon payment" copy; the **payment-confirmation** email's receipt button relabeled "View Receipt" + a note that the invoice arrives separately; and **Stripe was set up for live-mode** (operator swapped `sk_live_` keys + a live webhook — verify the box `.env` is `sk_live_` before HEMNET; test-mode was in place during dev).
- **Scheduled email — "one-shot, late-inclusive" + a fixed overlook** (June 30) — Organizer ask: a scheduled email should reach people who register *after* it's scheduled. It mostly already did (filter-based scheduled sends re-resolve recipients at fire time via `executeBulkEmail`), but a real **overlook** surfaced: the schedule-create route **parsed `recipientIds` but never wrote it** (only the immediate-send route persisted it), so *every* scheduled send silently fell back to filter-based — meaning a "schedule to these N ticked rows" actually fanned out to **everyone matching the filters** at fire time (often *all* registrations when no filter bar was set), an over-send risk on a live system. Fix (frontend-led, **no schema/migration** — `ScheduledEmail.recipientIds` already existed): the schedule route now persists `recipientIds` + exposes it on GET; the bulk-email dialog makes the choice **explicit** when scheduling from a row selection — default **"Everyone matching the current filters at send time"** (late-inclusive, drops the ids) vs **"Only the N selected (fixed list)"**; an "Email All" schedule shows a "✓ includes new registrations" note; the Scheduled Emails list labels each row **"matching at send time"** vs **"N fixed"**. "Send now" semantics unchanged. New test `__tests__/api/scheduled-email-create.test.ts` pins persistence. Full detail in [`docs/SCHEDULED_EMAILS.md`](SCHEDULED_EMAILS.md) §4; operator guide in `public/user-guide.html` §9 "Scheduled emails".
- **Communications filters + Activity edit-history diffs + public SEO + Add-form parity + faculty data fix** (June 29) — Operator-facing batch. The **"Send Bulk Email" dialog** (both the registrations-list "Email All" and the Communications page) gained **in-dialog multi-select** audience filters in a collapsible "Filter recipients" section — Payment status / **Registration type** / **Badge type** / **Tags** (`bulk-email.ts` `ticketTypeIds`/`badgeTypes`/`tagsInclude`; `bulk-email-dialog.tsx`, 4xl, 3-up); OR-within-field / AND-across; also fixed the bug where the list's "Filter by tag" was silently ignored by Email All. Then added a one-click **"Exclude faculty / speakers"** toggle (`excludeFaculty` → `EXCLUDE_FACULTY_WHERE`, `NOT ticketType.isFaculty`) and **removed the Communications page's duplicate single-select registration filters** so the dialog is the single registration-filter surface (the Speakers card keeps its own). The registration + speaker **Activity timeline now renders field-level before→after diffs** (incl. nested attendee fields), finance-redacted (`src/lib/activity-feed.ts`); a dedup follow-on extracted the shared activity types to a client-safe `src/lib/activity-feed-types.ts` and moved the two global audit feeds onto a shared `src/components/activity/audit-log-display.ts` (icon/colour maps + describe/actor helpers — components kept separate). Public `/e/[slug]/*` pages got **per-event + per-section SEO metadata** (server `layout.tsx` + `buildEventMetadata` in `src/lib/public-event-metadata.ts`; org-scoping is the multi-tenant follow-up). **Add Registration ↔ Add Speaker** now share one personal-field section (`PersonFormFields`; the speaker form was dropping Phone). Two **prod backfills** (both audited → show in the new Activity timeline): faculty `registrationType "Faculty"` → speaker profession or **default Physician** (33 rows; companion-creation path left unchanged), and speaker phone/additionalEmail enrichment (enrich-only, 1 row). `scheduled-email` 0-recipient is now a benign skip, not a paging FAILED. Plus: removed the confirmation-page "Back to Event" button, deleted the orphaned `docker/Dockerfile`, sidebar `w-64→w-56`.
- **Billing payers — event-level inline entry, org-level consolidation** (June 29) — A payer ("Charge to another account") can now be created **inline from the registration Charge-to picker** (the full-page Add Registration form) instead of the old 3-step Settings → Billing → create → attach → pick. Mirrors the Contact model: capture in context, dedupe into one org record. `findOrCreateBillingAccount()` in `billing-account-service.ts` — exact (case-insensitive, trimmed) name → **reuse** the existing org payer (never a duplicate); near-duplicate name → create + flag `needsReview` for an admin to merge later (advisory). New event-scoped `POST /api/events/[eventId]/billing-accounts` does the org-level find-or-create **and auto-attaches** the `EventBillingAccount` junction (`denyReviewer`+`denyFinance`). Reusable `AddPayerDialog` + `useCreateAndAttachBillingAccount`. The payer stays a single shared org row; only the per-event attachment is event-specific. **Deferred follow-ups** (reusable dialog makes them trivial): detail-sheet reassign picker, quick-add dialog, admin merge UI for `needsReview` payers — see `docs/ROADMAP.md`.
- **Data-loss / lost-update audit — 4-fix batch + event-clone completeness** (June 29) — A concurrency/data-loss sweep fixed: (1) EventsAir import now enrich-only (a sparse import can't wipe richer Contact data); (2) MCP accommodation re-book made atomic; (3) bulk-type + import-contacts got atomic oversell guards (`updateMany` compare-and-swap → 409 `CAPACITY_EXCEEDED`); (4) **`Event.settings`/`Organization.settings` lost-update** fixed via new `updateEventSettings`/`updateOrganizationSettings` helpers (`SELECT … FOR UPDATE` row-lock + merge inside a tx — holds across the pgbouncer transaction pooler) migrated across all ~13 settings writers. Separately, **contact sync is now enrich-only** (skip null/""), the **faculty registration-type bug** fixed (a speaker's companion shows their profession, never "Faculty"), and **event clone** now also copies custom `EmailTemplate` rows (survey config was already cloned; scheduled emails intentionally not).
- **Reviewer / submitter lifecycle hardening** (June 26) — A 3-agent end-to-end trace of the reviewer + submitter + crossover flows, shipped as a cluster of fixes: self-registered submitter-speakers now mint a companion "Faculty" registration (so they get badge / check-in / survey / cert like every other speaker); **resubmit-after-revision** re-stamps `submittedAt` + emails the author + notifies organizers (was silent); reviewers are now **notified** on per-abstract assignment (REST + MCP) and on pool-add (new `reviewer-assignment` / `reviewer-pool-invitation` email templates); **conflict-of-interest is now enforced** — a flagged reviewer is 403-blocked from submitting a review across REST + both MCP paths (was advisory-only); reviewer-invite **resend** + send-failure surfacing (502, not silent); and saving a **DRAFT** abstract no longer fires a "submitted" confirmation/admin-notify. Open items tracked in `docs/ROADMAP.md` "Reviewer/submitter lifecycle audit".
- **Hybrid mode change + correctness / RBAC / silent-failure batch** (June 24–26) — Admin can switch a registration **virtual↔in-person** with unified seat-accounting (`planSeatTransition`, capacity-guarded) + lazy entry-barcode minting on virtual→in-person (closing the last hybrid gap). Plus a correctness sweep: cert auto-issue **stall-reclaim is now `autoIssue`-aware** (survey-gated auto runs can't be stranded un-emailed after an SES/restart stall); several **silent failures now surface** (registrant portal shows an error card instead of a misleading "not registered" on fetch failure; bulk-tag + GET-load failures toast/show instead of swallowing); MEMBER write buttons gated on `canWrite` (no 403 surprises); media libraries gain a **grid/list view toggle** (per-user persisted, single-row list); plus assorted cert/webinar perf (latest-ref handlers, slower analytics poll).
- **Webinar waiting room — producer-gated admission + holding video + presence + 5k CDN** (June 23) — Attendees watch on our gated session page (not BigMarker). Registered attendees wait in a branded lobby (countdown + optional YouTube/Vimeo holding video); a **producer clicks "Open the room / Go live"** on the Webinar Console (sets the anchor `EventSession.status=LIVE`) to admit everyone at once into the event's chosen **viewing mode** — Zoom SDK embed (native Q&A) or custom HLS stream (5k via CloudFront). **Real-time presence:** new `WebinarPresence` table + heartbeat → a "Live now" console roster + a "Joined" registrations badge (distinct from post-event `ZoomAttendance`). Lobby config (`viewingMode`/`lobbyVideoUrl`/`lobbyMessage`) lives in `Event.settings.webinar`; new routes `/webinar/room`, `/webinar/presence`, public `/sessions/[id]/{lobby-status,presence}`; HLS is CDN-aware via `HLS_CDN_BASE` with origin failover. A two-agent adversarial review followed; all live/high/medium findings fixed in-band (presence `upsert` race, LivePlayer auto-recovery, close/re-open admit, hls-misconfig fallback, lobby-status micro-cache). Full architecture in [`docs/WEBINAR_EVENTS.md`](WEBINAR_EVENTS.md) "Waiting room…" section; operator guide in `public/user-guide.html` §11; deferred items + operator prerequisites (Zoom `sdkMode`→prod, verify box nginx `/stream/`, CloudFront+DR before 5k) in [`docs/ROADMAP.md`](ROADMAP.md).
- **Security / DDoS-bot hardening + Cloudflare removed** (June 10) — Decision: **no CDN/Cloudflare** (prod or DR). Fixed a real rate-limit bypass in `getClientIp()` (was trusting spoofable `CF-Connecting-IP` / first `X-Forwarded-For`; now prefers nginx `X-Real-IP` = the true socket peer). **Audit (read-only) confirmed no AWS WAF** anywhere in the account (nothing to attach to without an ALB/CDN) and **no host ModSecurity** — the "WAF on EC2" belief was a mix-up with fail2ban/ufw. **Shipped live: nginx per-IP rate limiting** (`limit_req`/`limit_conn` on `$binary_remote_addr`, 429 on exceed; runs before Node + survives restarts; tuned for the shared-NAT event case). **Added: fail2ban nginx jail** (bans IPs repeatedly tripping the 429 limit). Full verified posture + a complete "add Cloudflare later" playbook (the nginx `real_ip` approach needs zero app changes) live in [`docs/AWS_OPERATIONS.md`](AWS_OPERATIONS.md) §4. The live `/etc/nginx/sites-available/ea-sys` has diverged from the committed `deploy/nginx.conf` — the box is the source of truth for nginx config.
- **Certificates system — v3 PDF-overlay + multi-template + tag-driven Issue + cover-email editor** (June 2) — Major build over five commits (`b226f16` → `32a3610`). Designer hands the organizer a finished cert PDF (or PNG/JPG that gets server-converted via pdf-lib); the canvas editor (`pdfjs-dist` rasterize + `react-rnd` drag/resize) lets the operator position text boxes with `{{tokens}}` on top. N templates per category (ATTENDANCE | APPRECIATION) per event. Issuing is tag-driven manual selection — no auto-eligibility — and pops an email-editor dialog (Tiptap) pre-filled with the template's saved defaults or a system default for the category. Confirmed email content is snapshotted onto the run row so later template edits don't change in-flight emails. Two-phase cron pipeline (RENDER → AWAITING_REVIEW gate → SEND). Independent senior code review pass produced 4 BLOCKERs + 8 HIGHs — all blockers + 6 highs fixed in-band (path traversal + XSS escape + canvas PATCH spam debounce + draft-wipe-on-focus, plus failure-logging sweep + template-deleted-mid-run hard fail + transactional concurrent-run guard + sortOrder race fix). See `docs/CERTIFICATES.md` for the full architecture + the operator-facing guide at `public/user-guide.html` §18.
- **registration-detail-sheet.tsx staged refactor — A → E** (May 20) — Five careful, independently-revertable commits (`8ad760b` → `64dc640`) typed the picker hooks, extracted pure `toEditData`/`toServerPayload` mappers with 14 unit tests, introduced `ApiError` + method helpers (`src/lib/api-fetch.ts`) consolidating 5 mutationFn bodies into one-liners, fixed a latent prop-revert race via the React 19 "previous-prop snapshot" pattern, and pulled 5 inline handlers (incl. a 30-line badge-print async block) out to named functions. Sheet 2,174 → ~2,063 lines, +22 unit tests. F + G + H tracked in `docs/ROADMAP.md` with explicit trigger conditions. See section 14 "Known Gaps & Future Work".
- **Per-event payer scoping — EventBillingAccount junction** (May 20, `d7941d6`) — Many-to-many association via junction table with **shared identity**: one logical `BillingAccount` ("Cleveland Clinic") attached to many events without being duplicated. v1 was org-scoped and caused picker overflow; v2 introduces `EventBillingAccount(eventId, billingAccountId)` with `@@unique` + Cascade FKs from both ends. Pickers filter by junction membership (`?eventId=…`). Settings → Billing → BillingAccountsCard gets an "Events" count column + an `EventsAttachmentDialog` for attach/detach. Defensive migration backfill from existing Registration pairs (no-op for fresh installs).
- **"Charge to another account" v1 — BillingAccount payer** (May 19, `ea6d1bb`) — Doctors funded by their hospital or a pharma grant. New org-scoped `BillingAccount` model + `Registration.billingAccountId` / `payerReference` / `attendeeIsGuarantor`. **Orthogonal to `paymentStatus`** (deliberate anti-INCLUSIVE design): money is still owed, only the invoice bill-to party changes. Service: `src/services/billing-account-service.ts`. PDF bill-to renders the payer with the attendee on a reference line. Finance boundary: payer keys in `FINANCIAL_KEYS` so MEMBER never sees who funds a doctor (Mecomed-sensitive). UI: `BillingAccountsCard` (Settings → Billing), payer picker on Add Registration form, reassign/guarantor/revert on the detail sheet.
- **Multi-agent audit remediation — 2 BLOCKERs + 4 HIGHs** (May 18, `ff3b7e0`) — Supervisor + 4 specialist review agents (React, Prisma, backend/RBAC, architecture) found and remediated: cross-tenant IDOR on event email templates (5 handlers), `soldCount` never released on cancel via MCP/bulk (silent sold-out inflation), registrations LIST finance leak to MEMBER, contacts write-guard bypass (MEMBER + REGISTRANT could write contacts), PaymentStatus enum drift (INCLUSIVE unreachable via MCP), `getNextSerialId` race (replaced with `RegistrationSerialCounter` atomic upsert). +20 tests, suite 1175 → 1195.
- **Admin full pricing-tier control on manual add** (May 18, `434a45e`) — Manual Add Registration form now exposes all pricing tiers (active OR inactive), so organizers can record late registrants at the closed Early Bird rate as a courtesy. Public self-register still gates `isActive: true`. Documented tradeoffs in a block comment at `registration-service.ts` tier-validation.
- **`PaymentStatus.INCLUSIVE` + `Registration.sponsorId`** (May 1) — Sponsor-paid bulk-contract registrations (Abbott, Pfizer covering N attendees). Distinct from `COMPLIMENTARY` (VIP/staff no-charge). Violet badge. Sponsor reference into `Event.settings.sponsors[]`. CSV import gets `sponsor` column resolved by name.
- **Pricing tier visibility — manual add + list + dashboard** (May 14) — `Registration.pricingTierId` captured on both public Stripe-checkout and admin manual-add paths. New "Tier" column in registrations list + CSV. Event dashboard "Registrations by Tier" tile groups by tier name across all ticket types.

- **Email immutability + dedicated Change Email flow** (April 24) — Organizer-reported identity drift: the Speaker detail sheet's inline email input (and latent Contact PUT) updated only the entity row and left linked `User.email` + org Contact stale, silently splitting canonical identity across three tables. Fix locks `email` out of the general-purpose PUT schemas on Speaker / Registration / Contact (they return 400 `EMAIL_IMMUTABLE` rather than silently stripping) and adds three dedicated PATCH `/email` endpoints that do the full collision-check + User cascade + Contact re-sync atomically inside one `$transaction`. Registration PATCH **clones the shared Attendee** when it's linked to multiple registrations (public-register's orphan-reuse path can produce this) instead of mutating in place. Contact PATCH deliberately does not cascade to Speaker / User / Registration — Contact is a CRM snapshot, propagating from there would re-introduce the drift. Shared helper at `src/lib/email-change.ts`. New `ChangeEmailDialog` wired into all three detail sheets (disabled email field + Change button). 29 new tests (1055/1055 green). Commit `5da24e6`. See CLAUDE.md "Email immutability + dedicated Change Email flow" for the full surface + error codes.
- **Services layer — Phase 0 + 1 + 2a + 2b + 2c shipped** (April 22-23) — new `src/services/` directory holds shared domain logic called by both REST routes and MCP agent tools. **Phase 0** fixed confirmed drift bugs in the MCP tools (paid MCP registrations were silently skipping the confirmation email + quote PDF). **Phase 1** extracted `accommodation-service.ts` and locked in the conventions every subsequent service follows. **Phase 2a** extracted `abstract-service.ts` (status transitions + gate check + notifications). **Phase 2b** extracted `speaker-service.ts` (admin POST + MCP create). **Phase 2c** extracted `registration-service.ts` (admin POST + MCP create) — centralizes the atomic soldCount guard, paymentStatus defaulting, and the Phase 0 confirmation-email gate so the class of drift bug that motivated the refactor cannot recur. `src/services/README.md` is the full convention reference. Phase 3 (external public REST API) is the next forcing function — when its spec lands, new endpoints back onto the existing services. See section 2.5 "Services Layer" and section 14 "Services Refactor Status" below.
...
- **Zoom Web Embed v6** (April 15; React-19 fix June 23) — in-page Zoom Component View. The `/embedded` UMD externalizes React (it does NOT bundle it) and uses React-18's `ReactCurrentOwner`, which React 19 removed → it crashes on an npm import. Fixed by loading the SDK + its own React 18 from Zoom's CDN as isolated globals (`src/lib/zoom/load-embedded-sdk.ts`, `NEXT_PUBLIC_ZOOM_EMBED_LOADER=cdn|npm`).
...
# 5. Start development server (binds port 3113 per package.json — set NEXTAUTH_URL=http://localhost:3113 in .env to match)
npm run dev
# App runs at http://localhost:3113
```
...
**Session lifetime:** `maxAge: 48h`, declared ONCE as `SESSION_CONFIG` in `src/lib/auth.config.ts` and consumed by **both** NextAuth instances: the Node one in `auth.ts` and the Edge one in `proxy.ts` (`NextAuth(authConfig)`). **⚠ Corrected Aug 17 2026: before that date this section was wrong.** Only `auth.ts` set `maxAge`; `auth.config.ts` was silent, and a silent config does not inherit its sibling. It takes NextAuth's **default of 30 days**. Both instances re-issue the same cookie and middleware runs on every dashboard route, so the real lifetime was **30 days rolling**, not 24h. Found by asking why a session survived a weekend with the laptop shut down: `LoginEvent` showed the last sign-in on Aug 7 while `User.lastSeenAt` showed activity on Aug 17. Now one shared constant (drift pinned by `__tests__/lib/session-config.test.ts`), and 48h chosen deliberately: 24h re-prompts daily staff, a week is long for an account that moves money and cannot be force-signed-out. Note 48h does **not** carry a weekend (~63h Friday evening → Monday morning). The rest of this paragraph describes the sliding mechanism, which was and remains accurate: this is a **rolling (sliding) session, not a hard cap from login.** NextAuth v5's `GET /api/auth/session` path (`@auth/core/lib/actions/session.js`) re-signs the JWT with `exp = now + 48h` and re-sets the cookie **every time the session is read** (unconditionally — `updateAge` gates a different path), and the client `SessionProvider` reads the session on mount / tab focus / navigation. So **48h is an *idle* timeout** — the window slides forward on every use; an active user effectively never gets logged out, and only a full 48h of *inactivity* (app not opened/focused) expires the cookie. (The `maxAge` comment in `src/lib/auth.ts` previously said "forces re-authentication daily" — that was wrong; corrected.) **There is NO refresh token** for the dashboard session — it's a single signed JWT cookie. (Distinct from the MCP OAuth subsystem, which *does* have 30-day access / 90-day refresh tokens for external clients like claude.ai/n8n — see §MCP, not the dashboard login.) If a true hard daily logout is ever required (compliance), `maxAge`/`updateAge` won't deliver it — stamp `loginAt` in the JWT on sign-in and reject in the `jwt` callback once `now - loginAt > 24h`.
...
`@zoom/meetingsdk@^6.0.0` is in `serverExternalPackages`. Component View works under React 19 by **loading the SDK + its own React 18 from Zoom's CDN** (`src/lib/zoom/load-embedded-sdk.ts`, default `NEXT_PUBLIC_ZOOM_EMBED_LOADER=cdn`) — because the npm `/embedded` UMD externalizes React and uses React-18's `ReactCurrentOwner` (removed in React 19), so a plain `import "@zoom/meetingsdk/embedded"` crashes. Flip the env var to `npm` once Zoom ships React-19 support. The top-level `@zoom/meetingsdk` Client View entry is still blocked (it mounts into the host fiber tree). Full architectural detail in `docs/ZOOM_INTEGRATION.html` (local).
...
Two layers (prod is a directly-exposed EC2 — no CDN/proxy in front):

1. **App layer — `checkRateLimit`** (`src/lib/security.ts`): in-memory `Map` with periodic cleanup, ~80 buckets keyed by IP / `userId` / org / API-key / OAuth-token (the most specific identity available per endpoint). `getClientIp()` reads nginx's `X-Real-IP` (the true socket peer) — it deliberately does **not** trust `CF-Connecting-IP` or the first `X-Forwarded-For` entry (both client-spoofable on a directly-exposed origin).
2. **nginx layer — origin-side** (`/etc/nginx/sites-available/ea-sys` on the box; reference copy in `deploy/nginx.conf`): `limit_req`/`limit_conn` keyed on `$binary_remote_addr`, 429 on exceed. Runs **before** Node and **survives app restarts** (which the in-memory app limiter doesn't). A **fail2ban nginx jail** bans IPs that repeatedly trip the 429 limit. Full posture + tuning in [`docs/AWS_OPERATIONS.md`](AWS_OPERATIONS.md) §4.

**Known gap:** the app-layer in-memory state resets on deploy/restart and isn't shared across blue/green containers — the nginx layer covers that gap origin-side; a Redis (Upstash) store would make the app limiter durable + shared. Documented in `docs/VERCEL_COMPATIBILITY.md`.
...
### Services Refactor Status (as of April 23, 2026)
...
- **Phase 2c — Registration (shipped, commit `aac0381`).** `registration-service.ts` with `createRegistration()`. Covers REST admin POST + MCP single-create. 9 finite-union error codes, atomic soldCount guard with typed sentinel for tx-rollback discrimination, empty-string-to-null normalization on all optional attendee fields, and the Phase 0 confirmation-email gate structurally guaranteed by construction. Public register path + MCP bulk intentionally left inline — different concerns.
- **Phase 3 — External API-driven (pending).** When the public REST API ships, its endpoints back onto the existing services. New operations the API exposes get their own extractions at that point.
...
| `src/services/registration-service.ts` | `createRegistration()` — single-create path. Reused by REST POST `/api/events/[id]/registrations` and MCP `create_registration`. Owns: atomic soldCount guard with typed `RegistrationServiceSentinel`, paymentStatus defaulting (UNASSIGNED for paid, COMPLIMENTARY for free), Phase 0 confirmation-email gate (paid + outstanding). Public register (`/api/public/events/[slug]/register`) and MCP bulk (`create_registrations_bulk`) intentionally NOT in this service — different concerns. Phase 2c. |
