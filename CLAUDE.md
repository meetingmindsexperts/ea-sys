...
- `src/lib/agent/event-tools.ts` - Single source of truth for MCP + in-app AI Agent tools. 65 executors in `TOOL_EXECUTOR_MAP` across 10 domains: events (create_event, list_event_info, get_event_dashboard, get_event_stats, search_event), registrations (list, list_unpaid, create, check_in, update, bulk_update_status), speakers (list, create, update, list_agreements), sessions (list, create, update, list_live_now, add_topic), tracks, tickets, abstracts, hotels/accommodations (list_hotels/create_hotel/list_room_types/list_accommodations/create_accommodation/update_accommodation_status — full CRUD with atomic overbooking guard), media, zoom, webinar (info, attendance, engagement), sponsors (list/upsert), promo codes (CRUD), scheduled emails (list/cancel), contacts (list/create), invoices (list/create/send/update_status — REFUNDED is DB-only, not Stripe), email templates (list/update/reset), reviewers. **Every new tool here must be mirrored in `src/lib/agent/mcp-server-builder.ts` AND bump `package.json` version** (serves as MCP client cache-invalidation hint via `serverInfo.version`).
