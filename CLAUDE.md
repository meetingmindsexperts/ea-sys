...
- `src/lib/agent/event-tools.ts` - Single source of truth for MCP + in-app AI Agent tools. 57 executors in `TOOL_EXECUTOR_MAP` across 10 domains: events (create_event, list_event_info, get_event_dashboard, get_event_stats), registrations (list, list_unpaid, create, check_in, update, bulk_update_status), speakers (list, create, update, list_agreements), sessions (list, create, update, list_live_now, add_topic), tracks, tickets, abstracts, hotels/accommodations, media, zoom, webinar (info, attendance, engagement), sponsors, promo codes (CRUD), scheduled emails, contacts, invoices, email templates, reviewers, search_event
