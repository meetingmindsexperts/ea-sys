-- Row-Level Security policy: CrmActivity domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmActivity carries a
...
--     pg_policy, so it already covers CrmActivity with no code change.
--
...
DROP POLICY IF EXISTS crmactivity_tenant_isolation ON "CrmActivity";
CREATE POLICY crmactivity_tenant_isolation ON "CrmActivity"
  FOR ALL TO PUBLIC
