-- Row-Level Security policy: CrmCompany domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmCompany carries a
...
--     pg_policy, so it already covers CrmCompany with no code change.
--
...
ALTER TABLE "CrmCompany" ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS crmcompany_tenant_isolation ON "CrmCompany";
CREATE POLICY crmcompany_tenant_isolation ON "CrmCompany"
  FOR ALL TO PUBLIC
