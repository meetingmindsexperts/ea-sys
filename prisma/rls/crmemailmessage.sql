-- Row-Level Security policy: CrmEmailMessage domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmEmailMessage carries a
...
ALTER TABLE "CrmEmailMessage" ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS crmemailmessage_tenant_isolation ON "CrmEmailMessage";
CREATE POLICY crmemailmessage_tenant_isolation ON "CrmEmailMessage"
  FOR ALL TO PUBLIC
