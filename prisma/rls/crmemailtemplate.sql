-- Row-Level Security policy: CrmEmailTemplate domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmEmailTemplate carries a
...
--     pg_policy, so it already covers CrmEmailTemplate with no code change.
--
...
ALTER TABLE "CrmEmailTemplate" ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS crmemailtemplate_tenant_isolation ON "CrmEmailTemplate";
CREATE POLICY crmemailtemplate_tenant_isolation ON "CrmEmailTemplate"
  FOR ALL TO PUBLIC
