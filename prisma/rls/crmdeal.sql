-- Row-Level Security policy: CrmDeal domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmDeal carries a
...
DROP POLICY IF EXISTS crmdeal_tenant_isolation ON "CrmDeal";
CREATE POLICY crmdeal_tenant_isolation ON "CrmDeal"
  FOR ALL TO PUBLIC
