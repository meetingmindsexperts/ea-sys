-- Row-Level Security policy: CrmPipelineStage domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmPipelineStage carries a
...
--     pg_policy, so it already covers CrmPipelineStage with no code change.
--
...
DROP POLICY IF EXISTS crmpipelinestage_tenant_isolation ON "CrmPipelineStage";
CREATE POLICY crmpipelinestage_tenant_isolation ON "CrmPipelineStage"
  FOR ALL TO PUBLIC
