-- Row-Level Security policy: CrmEmailSendClaim domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmEmailSendClaim carries a
...
--     pg_policy, so it already covers CrmEmailSendClaim with no code change.
--
...
ALTER TABLE "CrmEmailSendClaim" ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS crmemailsendclaim_tenant_isolation ON "CrmEmailSendClaim";
CREATE POLICY crmemailsendclaim_tenant_isolation ON "CrmEmailSendClaim"
  FOR ALL TO PUBLIC
