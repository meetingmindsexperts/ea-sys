-- Row-Level Security policy: CrmProduct domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmProduct carries a
...
--     pg_policy, so it already covers CrmProduct with no code change.
--
...
DROP POLICY IF EXISTS crmproduct_tenant_isolation ON "CrmProduct";
CREATE POLICY crmproduct_tenant_isolation ON "CrmProduct"
  FOR ALL TO PUBLIC
