-- Row-Level Security policy: CrmTask domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmTask carries a
...
--     pg_policy, so it already covers CrmTask with no code change.
--
...
DROP POLICY IF EXISTS crmtask_tenant_isolation ON "CrmTask";
CREATE POLICY crmtask_tenant_isolation ON "CrmTask"
  FOR ALL TO PUBLIC
