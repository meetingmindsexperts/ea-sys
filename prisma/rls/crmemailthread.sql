-- Row-Level Security policy: CrmEmailThread domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmEmailThread carries a
...
ALTER TABLE "CrmEmailThread" ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS crmemailthread_tenant_isolation ON "CrmEmailThread";
CREATE POLICY crmemailthread_tenant_isolation ON "CrmEmailThread"
  FOR ALL TO PUBLIC
