-- Row-Level Security policy: CrmNote domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmNote carries a
...
--     pg_policy, so it already covers CrmNote with no code change.
--
...
DROP POLICY IF EXISTS crmnote_tenant_isolation ON "CrmNote";
CREATE POLICY crmnote_tenant_isolation ON "CrmNote"
  FOR ALL TO PUBLIC
