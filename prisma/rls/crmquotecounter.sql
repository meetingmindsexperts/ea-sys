-- Row-Level Security policy: CrmQuoteCounter domain (CRM full-domain sweep, policy layer
...
--   * FLAT policy on the row's own organizationId column (CrmQuoteCounter carries a
...
--     pg_policy, so it already covers CrmQuoteCounter with no code change.
--
...
ALTER TABLE "CrmQuoteCounter" ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS crmquotecounter_tenant_isolation ON "CrmQuoteCounter";
CREATE POLICY crmquotecounter_tenant_isolation ON "CrmQuoteCounter"
  FOR ALL TO PUBLIC
