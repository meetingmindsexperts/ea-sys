...
const CASES: CrmRlsCase[] = [
  ...
  {
    name: "CrmQuote",
    scopedRead: () => db.crmQuote.findMany({ select: { organizationId: true } }),
    findB: () => db.crmQuote.findUnique({ where: { id: CRM_QUOTE_B_ID }, select: { id: true } }),
    failClosed: () => db.crmQuote.findMany({ select: { id: true } }),
    deleteB: () => db.crmQuote.delete({ where: { id: CRM_QUOTE_B_ID } }),
    reHome: () => db.crmQuote.update({ where: { id: CRM_QUOTE_A_ID }, data: { organizationId: ORG_B_ID } }),
  },
  {
    name: "CrmQuoteLine",
    scopedRead: () => db.crmQuoteLine.findMany({ select: { organizationId: true } }),
    findB: () => db.crmQuoteLine.findUnique({ where: { id: CRM_QUOTE_LINE_B_ID }, select: { id: true } }),
    failClosed: () => db.crmQuoteLine.findMany({ select: { id: true } }),
    deleteB: () => db.crmQuoteLine.delete({ where: { id: CRM_QUOTE_LINE_B_ID } }),
    reHome: () => db.crmQuoteLine.update({ where: { id: CRM_QUOTE_LINE_A_ID }, data: { organizationId: ORG_B_ID } }),
  },
  {
    name: "CrmEmailThread",
