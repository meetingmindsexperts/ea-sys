...
/**
 * GET /api/crm/contacts/tags — the org's DISTINCT CRM-contact tags.
 *
 * Powers the "Any tag" filter on the contacts list. Computed over the whole
 * non-archived org book (NOT the filtered view) so the filter always offers
 * every tag, even while a tag filter is active. Returns exact stored strings, so
 * the list's `?tags=` filter (an `hasSome` match) never mismatches on casing.
 */
export async function GET(req: Request) {
  ...
  return await runWithTenant(ctx.organizationId, async () => {
    try {
      ...
      apiLogger.error({
        msg: "crm/contacts/tags:list-failed",
