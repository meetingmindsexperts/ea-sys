/** POST approve or reject a PROPOSED supplier (the settle holder, a super admin or the final approver; a conditional claim, decided once). */
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ supplierId: string }> }) {
  const [g, { supplierId }] = await Promise.all([procurementGuard({ route: ROUTE, need: "decide-supplier", write: true }), params]);
