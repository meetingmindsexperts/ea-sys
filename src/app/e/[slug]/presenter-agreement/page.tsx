...
function PresenterAgreementContent() {
  ...
  const fetchAgreement = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/public/events/${slug}/presenter-agreement?token=${encodeURIComponent(token)}`);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/public/events/${slug}/presenter-agreement`, {
        method: "POST",
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-6">
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
          ...
          <div className="px-6 sm:px-10 py-6 border-b border-slate-100">
            <h2 className="text-2xl font-bold text-slate-900">Presenter Agreement</h2>
            <p className="text-sm text-slate-500 mt-1">
              Please review the agreement below and confirm your acceptance to present your accepted abstract(s) at this event.
            </p>
          </div>

          <div className="p-6 sm:px-10 space-y-6">
            {/* Presenter Info (read-only) */}
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
              <p className="text-xs uppercase tracking-wider text-slate-400 mb-1">Presenter</p>
            ...
            {isAccepted ? (
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-6 text-center">
                ...
                <p className="text-sm text-emerald-700">
                  Thank you! Your presenter agreement has been recorded.
                  {data.acceptedAt && (
                    <>
                ...
                <div className="rounded-lg border border-slate-200 bg-white p-4">
                  <label htmlFor="accept-agreement" className="flex items-start gap-3 cursor-pointer">
                    ...
                    <Label htmlFor="accept-agreement" className="text-sm text-slate-700 leading-relaxed cursor-pointer">
                      I have read and agree to the Presenter Agreement for <strong>{data.event.name}</strong>.
...
export default function PresenterAgreementPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-white flex items-center justify-center">
