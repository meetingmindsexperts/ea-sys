...
// The Event Agent chat, shared by /agent (organisation level, event optional)
// and /events/[eventId]/agent (the same conversation opened from an event).
// Requests go to POST /api/agent/execute with the event in the body; history
// stays in the browser until runs are stored (architecture review §4.6).
...
const TOOL_LABELS: Record<string, string> = {
  ...
  get_event_info: "Getting event info",
  list_events: "Listing events",
  search_event: "Searching the event",
...
function getToolResultSummary(name: string, result: unknown): string {
  ...
  if ((name === "list_event_info" || name === "get_event_info") && r.name) return `Fetched event "${r.name}"`;
  if (name === "create_event" && r.event) return `Created event "${(r.event as { name: string }).name}"`;
  if (name === "list_events" && Array.isArray(r.events))
    return `Found ${r.events.length} event${r.events.length !== 1 ? "s" : ""}`;

  return "Completed";
}

const EVENT_SUGGESTED_COMMANDS = [
...
const ORG_SUGGESTED_COMMANDS = [
  "Which events start in the next 60 days?",
  "Create a draft event called Gulf Cardiology Summit, 12 to 14 March 2027, Dubai",
  "How many confirmed registrations does each published event have?",
  "Find the contact for dr.example@hospital.org and list the events they attended",
  "List the speakers on the next event who have not signed their agreement",
];

// ─── localStorage helpers ─────────────────────────────────────────────────────

function storageKey(eventId: string | null) {
  return `agent-session-${eventId ?? "org"}`;
}

function loadMessages(eventId: string | null): ChatMessage[] {
  try {
...
export function AgentChat({
  ...
  /** The event the conversation is about; null for the organisation-level door. */
  eventId: string | null;
  /** Rendered beside the title: the event picker or a link to the other door. */
              ...
              <div className="flex flex-col items-center justify-center h-full py-16 text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  {eventId
                    ...
                    : "Ask about any event, or create one. Name the event and the agent finds it."}
                </p>
              </div>
            )}
            {messages.map((msg) => {
        ...
        <div className="space-y-4 overflow-y-auto">
          <Card>
            ...
            <CardContent className="space-y-1.5 pt-0">
              {(eventId ? EVENT_SUGGESTED_COMMANDS : ORG_SUGGESTED_COMMANDS).map((cmd) => (
                <button
                  key={cmd}
          ...
          <Card>
            ...
            <CardContent className="pt-0">
              <ul className="text-xs text-muted-foreground space-y-1">
                <li>✓ Reads and creates across events, tracks, sessions, speakers, registrations, contacts and sponsors</li>
                <li>✓ Creates and updates events (dates and status stay in Settings)</li>
                <li>✓ Sends bulk email, after you confirm the count</li>
                <li>✓ CRM and Budgets tools appear for the roles that hold them</li>
                <li>✗ Never deletes a record, refunds money or touches user accounts</li>
              </ul>
            </CardContent>
          </Card>
        </div>
