...
- **Reports & export** — pipeline-by-stage, win/loss + win rate, per-rep leaderboard, a group-by breakdown (pipeline / rep / event / deal type / lost reason / expected-close month / closed month; `bucketDeals` in `lib/reports.ts`, one row fetch under the report's own `where`), and CSV export of the filtered deal list (values finance-gated).
...
**Real-DB integration tests (`npm run test:integration-db`):** the mocked suite
...
junction `organizationId`. Those go in `tests/integration-db/*.test.ts` (the shared real-Postgres harness,
also home to the procurement, worker and registration suites), which run against
a REAL Postgres (`db` pointed at a `crm_test` database on the shared test
container — `docker compose --profile crm-test up -d`, then `INTEGRATION_TEST_DATABASE_URL`
