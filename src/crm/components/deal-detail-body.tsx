 ...
 *
 * This is the BODY of the dedicated deal page (/crm/deals/[dealId]). It used to live
 * inside a slide-out sheet; it now renders full-width on its own route, so a deal is
 * deep-linkable and the back button behaves. `onClosed` is called after an archive
 * or a won/lost close so the page can navigate back to the board.
 *
 * Notes are the reason this view exists. Everything else in EA-SYS's activity trail
 * is derived (emails sent, fields changed); a note is a human saying "I called them,
 * they want Gold, decision after the board meets" — the one thing no automated sync
 * can produce, and why the pipeline is worth more than a spreadsheet.
 *
 * A note may only be edited by its author, so we render the delete affordance only
 * where it will actually be honoured — a button that always 403s is worse than none.
 */
      ...
      {canWrite && !isClosed && (
        <div className="space-y-3 rounded-lg border p-4">
          ...
          <div className="flex gap-2">
            <Button size="sm" onClick={() => handleClose("WON")} disabled={closing} className="flex-1">
              {closing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Mark won
            </Button>
            <Button size="sm" variant="outline" onClick={() => handleClose("LOST")} disabled={closing} className="flex-1">
              Mark lost
            </Button>
          </div>
        </div>
      ...
      {canWrite && (
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={handleAddTask} disabled={!taskTitle.trim() || createTask.isPending}>
              Add
            </Button>
          </div>
        ...
        {canWrite && (
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={handleAddNote} disabled={!noteBody.trim() || createNote.isPending}>
 ...
 * table; putting a doctor here would be a category error, and putting a rep in THERE
 * would push them into the HCP marketing mirror.
 */
