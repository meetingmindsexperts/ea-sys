...
/**
 * Variables that are ALWAYS server-built HTML blocks (never user input), so
 * every caller renders them raw without opting in. Derived from the registry:
 * the block tokens usable on any template plus each template's own. Keep the
 * set strictly to fields built from template literals in code (with their
 * inner user-input pieces already individually `escapeHtml`'d); user-controlled
 * strings like `personalMessage` stay caller-managed.
 */
const DEFAULT_RAW_HTML_KEYS = new Set<string>([
  ...GLOBAL_RAW_HTML_KEYS,
  ...EMAIL_TEMPLATE_SPECS.flatMap((t) => t.rawHtmlKeys),
]);
...
/** Each template's own variables (beyond the global event block), from the registry. */
export const TEMPLATE_VARIABLES: Record<string, { key: string; description: string }[]> = Object.fromEntries(
  EMAIL_TEMPLATE_SPECS.map((t) => [t.slug, [...t.variables]]),
);

// ── Default templates (bodies in email-template-defaults.ts) ───────────────────
...
/** Name + default body per system template, in seed order. */
export const DEFAULT_TEMPLATES: DefaultTemplate[] = EMAIL_TEMPLATE_SPECS.map((t) => ({
  slug: t.slug,
  name: t.name,
  ...DEFAULT_TEMPLATE_BODIES[t.slug],
}));
...
<div style="margin:16px 0;">
    ...
    coAuthorNames: "John Smith, Mary Johnson",
    newStatus: "ACCEPTED",
